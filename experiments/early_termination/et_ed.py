...
def conf_mat_bsds500(bsds_id, load=False):
    if load:
        ...
        np.save("results/bsds500_{}_SC.npy".format(bsds_id), np.array(SC_vals).reshape(320, 480) * 512)
        np.save("results/bsds500_{}_cape.npy".format(bsds_id), np.array(cape_et_vals).reshape(320, 480) * 512)
        np.save("results/bsds500_{}_var.npy".format(bsds_id), np.array(var_et_vals).reshape(320, 480) * 512)
