...
from sim.PTV import SCC_from_rho_bv_gaussian
...
class NONINT_RAND_SNG(SNG):
    """Pair of RAND SNGs, one that generates an nxn +1 correlation matrix, the other generates nxn identity matrix
        output bitstreams are generated by muxing the two bitstreams together according to the desired non-integer SCC
    """
    def __init__(self, w, scc, nv, nc=0):
        self.scc = scc
        self.sng_corr = RAND_SNG(w, np.ones((nv, nv)))
        self.sng_uncorr = RAND_SNG(w, np.identity(nv), nc)
        super().__init__(self.sng_uncorr.rns, np.identity(nv), w, nc)

    def run(self, parr, N):
        bs_corr = self.sng_corr.run(parr, N)
        bs_uncorr = self.sng_uncorr.run(parr, N)
        bs_combined = nonint_scc(bs_uncorr[:self.nv, :], bs_corr, self.scc)
        bs_final = np.zeros((self.nv + self.nc, N), dtype=np.bool_)
        bs_final[:self.nv, :] = bs_combined
        bs_final[self.nv:, :] = bs_uncorr[self.nv:, :]
        return bs_final

class NONINT_LFSR_SNG_A(SNG):
    """Given an A matrix and the sign vector s, generate the corresponding bitstreams using LFSR RNS sources"""

    def __init__(self, A, s, w):
        #get_acoeffs_from_C produces a lower-left triangular matrix where rows represent the SNs and columns represent the RNS sources
        #In the proposal, the matrix is tranposed.
        #total number of component streams necessary is the number of non-zero elements in the A matrix
        self.A = A
        self.s = s

    def run(self, parr, N):
        #first get an augmented parr where Pxi is duplicated the correct number of times
        parr_aug = []
        for i in range(self.A.shape[0]):
            for j in range(self.A.shape[1]):
                if self.A[i, j] != 0:
                    parr_aug.append(parr[i])
        #here, we can form an integer correlation matrix and then use it to generate the bitstreams for parr_aug

        #after generating the bitstreams for parr_aug, we can perform the muxing here to get the final bitstreams

class GAUSSIAN_COPULA_SNG(SNG):
    def __init__(self, Cin):
        self.Cin = Cin

    def run(self, parr, N):
        bs_mat = generate_correlated_bernoulli(N, parr, self.Cin)
        return bs_mat
...
def generate_correlated_bernoulli(n_samples, p, C):
    """
    Generate Bernoulli variables using Gaussian copula

    Parameters:
        n_samples : int
        p         : array-like of probabilities (length n)
        C         : target correlation matrix (n x n)

    Returns:
        X : (n_samples x n) array of Bernoulli samples
    """
        ...
        for j in range(i):
            ...
            if np.round(Cij) == Cij:
                ...
                rho_soln = optimize.brentq(
                    lambda r: SCC_from_rho_bv_gaussian(r, p[i], p[j]) - Cij,
                    -1+1e-6, 1-1e-6
                )
            rho[i, j] = rho_soln
            rho[j, i] = rho_soln
