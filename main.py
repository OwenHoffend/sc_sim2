from sim.sim import *
from sim.SNG import *
from sim.PCC import *
from sim.SCC import *
from sim.RNS import *
from sim.datasets import *
from sim.circs.circs import *
from sim.PTV import *
from sim.PTM import *
from synth.COOPT import *
from synth.sat import *
from synth.experiments.joint_area_example import *
from synth.experiments.example_circuits_for_proposal import *
from synth.experiments.subcircuit_ptm_example import *
from synth.experiments.proposal_analysis_examples import *
from synth.unit_tests.run_synth_tests import *
from sim.PTV import get_Q
from experiments.early_termination.old_earlytermination.early_termination_plots import *
from experiments.early_termination.ET_on_images import *
import matplotlib.pyplot as plt
import numpy as np
from symb_analysis.sympy_test import *
from sim.sim import sim_circ
from experiments.sequential.scmc_test import sim_fsm_sync, test_CAP_fsm_sync, synchronizer_symbolic_curves, synchronizer_symbolic_error, sim_fsm_sync_px_sweep
from sim.circs.SCMCs import C_FSM_SYNC
from symb_analysis.experiments.test_seq_cap import *
from symb_analysis.seq_CAP import get_DV_symbols
from experiments.sequential.test_autocorr_bitstreams import *
from experiments.test_nonint_scc_gen import *
from symb_analysis.experiments.subcirc_ptm import *
from sim.copula_modeling import *
from synth.experiments.copula_paper_examples import *
from synth.unit_tests.COOPT_tests import *
import unittest

if __name__ == "__main__":

    from dissertation.pseudocode import get_Q as get_Q_pseudocode
    from dissertation.pseudocode import get_MV_via_copula
    p = get_MV_via_copula(None, np.array([0.5, 0.5]), 2)
    print(p)

    #gauss_copula_vs_linear_comb()
    #COOPT_example_3output()
    #s = [1, -1, -1, 1]
    #C = np.diag(s) @ (0.6 * np.eye(4) + 0.3 * np.ones((4,4)) + 0.1 * np.array([
    #    [1, 1, 0, 0],
    #    [1, 1, 0, 0],
    #    [0, 0, 1, 1],
    #    [0, 0, 1, 1]
    #])) @ np.diag(s)
    #print(C)
    #v = get_DV_via_copula(C, np.array([0.3, 0.4, 0.5, 0.6]), force_FH=True)
    #P, C = get_C_from_v(v, return_P=True)
    #print(P)
    #print(np.round(C, 4))

    #s = [1, -1, 1]
    #C = np.diag(s) @ (0.6 * np.eye(3) + 0.3 * np.ones((3,3)) + 0.1 * np.array([
    #    [1, 1, 0],
    #    [1, 1, 0],
    #    [0, 0, 1]
    #])) @ np.diag(s)
    #print(C)
    #v = get_DV_via_copula(C, np.array([0.3, 0.4, 0.5]), force_FH=True)
    #P, C = get_C_from_v(v, return_P=True)
    #print(P)
    #print(np.round(C, 4))

    #s = [-1, 1, 1]
    #C = np.array([
    #    [1, 1, 0],
    #    [1, 1, 0],
    #    [0, 0, 1]
    #])
    #print(C)
    #v = get_DV_via_copula(C, np.array([0.3, 0.4, 0.5]), force_FH=True)
    #P, C = get_C_from_v(v, return_P=True)
    #print(P)
    #print(np.round(C, 4))

    #A = np.array([[1.0, 0.0, 0.0],
    #              [0.4, 0.6, 0.0],
    #              [0.3, 0.3, 0.4]])
    #s = np.array([1, -1, 1])
    #sng = NONINT_LFSR_SNG_A(A, s, 16)
    #parr = np.array([0.3, 0.4, 0.5])
    #N = 1000
    #bs = sng.run(parr, N)

    #C = np.array([
    #    [ 1.000000, -0.087729, -0.199256,  0.487608],
    #    [-0.087729,  1.000000,  0.425952,  0.052918],
    #    [-0.199256,  0.425952,  1.000000,  0.275414],
    #    [ 0.487608,  0.052918,  0.275414,  1.000000],
    #])
    #v = get_DV_via_copula(C, np.array([0.5, 0.5, 0.5, 0.5]))
    #P, C = get_C_from_v(v, return_P=True)
    #print(P)
    #print(np.round(C, 4))

    C = np.array([
        [1, 0.5, 0.5],
        [0.5, 1, 0],
        [0.5, 0, 1]
    ])

    #v = get_DV_via_copula(C, np.array([0.5, 0.5, 0.5]), force_FH=True)
    #P, C = get_C_from_v(v, return_P=True)
    #print(P)
    #print(np.round(C, 4))

    p = np.full(3, 0.5)

    R_fit, result = fit_gaussian_copula_corr(C, p)

    print("Optimization loss:", result.fun)
    print("Eigenvalues:", np.linalg.eigvalsh(R_fit))
    print(R_fit.round(4))